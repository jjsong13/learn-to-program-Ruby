...
to exit from the loop. (Well, it will get you out of the whole method,
 but it will get you out of the loop in the process.) How do you like
...
def ask question
  while true
    ...
    reply = gets.chomp.downcase
    if (reply == 'yes' || reply == 'no')
      if question == 'Do you wet the bed?'
        if reply == 'yes'
          return true
        else #reply is 'no'
          return false
